...
it('flushes PCM chunks in order and releases the worklet', async () => {
  const node: {port: {onmessage: ((event: {data: unknown}) => void) | null; postMessage: ReturnType<typeof vi.fn>; close: ReturnType<typeof vi.fn>}; connect: ReturnType<typeof vi.fn>; disconnect: ReturnType<typeof vi.fn>} = {port:{onmessage:null,postMessage:vi.fn(),close:vi.fn()},connect:vi.fn(),disconnect:vi.fn()};
