...
afterEach(()=>{vi.unstubAllGlobals();vi.restoreAllMocks();clearSession();});
...
it('does not attach an old recorder when AudioContext.resume finishes after reset',async()=>{
  ...
  const resume=vi.fn().mockReturnValueOnce(oldResume).mockResolvedValue(undefined);
  const tracks:{stop:ReturnType<typeof vi.fn>;readyState:string}[]=[];
  const getUserMedia=vi.fn(async()=>{
    const track={stop:vi.fn(),readyState:'live'};tracks.push(track);
    track.stop.mockImplementation(()=>{track.readyState='ended';});
    return {getTracks:()=>[track]};
  });
  vi.stubGlobal('navigator',{mediaDevices:{getUserMedia}});
  vi.stubGlobal('AudioContext',class {
    resume=resume;close=vi.fn().mockResolvedValue(undefined);
  ...
  vi.stubGlobal('MediaRecorder',class {
    ...
    state='inactive';onstop: (()=>void)|null=null;
  ...
  function Harness(){recorder=useAudioRecorder({});return null;}
  const root=createRoot(document.createElement('div'));
  await act(async()=>root.render(<Harness/>));
  let oldStart!:Promise<void>;
  await act(async()=>{oldStart=recorder.startRecording();});
  await act(async()=>{recorder.reset();});
  await act(async()=>{await recorder.startRecording();});
  await act(async()=>{resumeOld();await oldStart;});
